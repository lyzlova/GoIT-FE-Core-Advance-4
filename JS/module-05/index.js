...
const getUsersByEyeColor = (arr, color) =>
  users.filter(x => x.eyeColor === color);
...
const getUsersByGender = (arr, gender) =>
  arr.filter(x => x.gender === gender).map(x => x.name);
...
const getUsersWithAge = (arr, min, max) =>
  arr.filter(obj => obj.age >= min && obj.age <= max);
...
const getUsersByFriend = (arr, name) => {
  const result = arr.reduce(
    (arr, user) => (user.friends.includes(name) ? arr.concat(user.name) : arr),
    [],
  );
  return result;
};
...
const getTags = arr =>
  arr.reduce((acc, x) => (x.skills ? acc.concat(x.skills) : acc), []);

const skills = getTags(users);
...
const countTags = arr => arr.reduce(getTagStats, []).sort();
console.log(countTags(skills));
//
